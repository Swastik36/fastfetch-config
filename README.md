...
- Centered layout: logo gap stays 0–2, extra width becomes equal left/right margins so the whole block (title, logo, values) stays centered on resize
...
- **Logo padding**: `"logo": { "padding": { "top": 8, "right": 2 } }` in the configs is the bare-fastfetch default. The engine overrides it per resize: right gap = `clamp(cols − 118, 0, 2)`, and `--logo-padding-left` centers the whole block (`max(0, (cols − content_width) / 2)`, where content width comes from the layout scan).
